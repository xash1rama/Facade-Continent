document.addEventListener("DOMContentLoaded", function() {
    fetch('/admin_portfolio_facade_gallery')
...
document.getElementById('createPortfolioButton').addEventListener('click', function() {
    ...
    if (portfolioId) {
        ...
        fetch(`/admin_portfolio_facade/${portfolioId}`)
            .then(response => response.json())
            .then(data => {
...
document.getElementById('changeMainImageButton').addEventListener('click', function() {
    ...
    fetch(`/admin_portfolio_facade/${portfolioId}/main-image`, {
        method: 'PUT',
        body: formData
    })
    .then(response => {
...
function uploadPhotos(portfolioId) {
    ...
    input.onchange = () => {
        ...
        fetch(`/admin_portfolio_facade/${portfolioId}/photos`, {
            method: 'POST',
            body: formData
        })
        .then(response => {
    ...
    function deletePhoto(portfolioId, filename, button) {
        ...
        fetch(`/admin_portfolio_facade/${portfolioId}/photos/${filename}`, {
            method: 'DELETE'
        })
        .then(response => {
...
document.getElementById('createPortfolioForm').addEventListener('submit', function(event) {
    ...
    fetch(`/admin_create_portfolio_facade`, {
        method: 'POST',
        body: formData
    })
    .then(response => {
...
function closeModal() {
    ...
    if (confirm("Вы уверены, что хотите удалить это портфолио?")) {
        fetch(`/admin_delete_portfolio_facade/${portfolioId}`, {
            method: 'DELETE'
        })
        .then(response => {
            if (!response.ok) {
